...
ASEAN_ISO2 = ";".join(ASEAN_COUNTRIES.keys())
...
def fetch_indicator(indicator: str, start: int, end: int) -> list[dict]:
    url = f"{BASE_URL}/country/{ASEAN_ISO2}/indicator/{indicator}"
    params = {
        "format":   "json",
        "date":     f"{start}:{end}",
        "per_page": 500,
...
def records_to_dataframe(records: list[dict], indicator_code: str) -> pd.DataFrame:
    ...
    for r in records:
        if r.get("value") is None:
            continue
        iso2 = r["country"]["id"]
        if iso2 not in ASEAN_COUNTRIES:
            continue
        ...
        rows.append({
            ...
            "iso2":           iso2,
            "iso3":           ASEAN_COUNTRIES[iso2],   # ← join key ke dim_negara
    ...
    if not df.empty:
        print(f"[reader_worldbank] {indicator_code}: {len(df)} baris valid "
              f"| negara: {sorted(df['iso3'].unique().tolist())}")
    else:
        print(f"[reader_worldbank] {indicator_code}: tidak ada baris valid")
    return df
...
def main():
    ...
    for ind in indicators:
        ...
        if raw_records:
            ...
            df = records_to_dataframe(raw_records, ind)
            if not df.empty:
                all_dfs.append(df)
        else:
    ...
    if all_dfs:
        ...
        csv_path = RAW_DIR / f"worldbank_batch_{args.batch}_combined.csv"
        df_all.to_csv(csv_path, index=False)
        print(f"\n[reader_worldbank] Combined CSV -> {csv_path}")
        print(f"[reader_worldbank] Total baris: {len(df_all)}")
        print(f"[reader_worldbank] Negara: {sorted(df_all['iso3'].unique().tolist())}")
        print(f"[reader_worldbank] Tahun: {sorted(df_all['year'].unique().tolist())}")
        print(f"[reader_worldbank] Indikator: {sorted(df_all['indicator_code'].unique().tolist())}")
    else:
        print("[reader_worldbank] Tidak ada data yang berhasil di-fetch")
